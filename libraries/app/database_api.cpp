...
{
   public:
      database_api_impl( const core::app::api_context& ctx  );
      ~database_api_impl();
      ...
      core::chain::database&                _db;
      std::shared_ptr< core::follow::follow_api > _follow_api;
...
database_api::database_api( const core::app::api_context& ctx )
   : my( new database_api_impl( ctx ) ) {}

database_api::~database_api() {}

database_api_impl::database_api_impl( const core::app::api_context& ctx )
   : _db( *ctx.app.chain_database() )
{
   ...
   try
   {
      ...
      _follow_api = std::make_shared< core::follow::follow_api >( ctx );
   }
...
{
   return core::protocol::get_config();
}
...
vector<account_id_type> database_api_impl::get_account_references( account_id_type account_id )const
{
   ...
   const auto& refs = aidx.get_secondary_index<core::chain::account_member_index>();
...
//   wdump((trx)(available_keys));
   auto result = trx.get_required_signatures( CORE_CHAIN_ID,
                                              available_keys,
...
{
   trx.verify_authority( CORE_CHAIN_ID,
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).active  ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).owner   ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).posting ); },
                         CORE_MAX_SIG_CHECK_DEPTH );
...
void database_api::set_pending_payout( discussion& d )const
{
   ...
   asset pot;
   if( my->_db.has_hardfork( CORE_HARDFORK_0_17__774 ) )
...
//   if( !hist.current_median_history.is_null() ) pot = pot * hist.current_median_history;
   ...
   if( total_r2 > 0 )
   {
      ...
      if( my->_db.has_hardfork( CORE_HARDFORK_0_17__774 ) )
      {
         ...
         vshares = d.net_rshares.value > 0 ? core::chain::util::evaluate_reward_curve( d.net_rshares.value, rf.author_reward_curve, rf.content_constant ) : 0;
      }
      else
         vshares = d.net_rshares.value > 0 ? core::chain::util::evaluate_reward_curve( d.net_rshares.value ) : 0;
   ...
   if( d.parent_author != CORE_ROOT_POST_PARENT )
...
vector<discussion> database_api::get_discussions_by_promoted( const discussion_query& query )const
{
   return my->_db.with_read_lock( [&]()
   {
      ...
      auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type(CORE_MAX_SHARE_SUPPLY) )  );
