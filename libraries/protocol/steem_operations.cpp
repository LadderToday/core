...
namespace core { namespace protocol {
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries[0].weight <= CORE_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= CORE_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= CORE_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   {
      validate_account_name( author );
      FC_ASSERT( percent_steem_dollars <= CORE_100_PERCENT, "Percent cannot exceed 100%" );
...
// dont care symbol is SBD or STEEM
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= CORE_100_PERCENT, "Weight is not a STEEMIT percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( memo.size() < CORE_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   void set_withdraw_vesting_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= CORE_100_PERCENT, "Percent must be valid core percent" );
   }
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( CORE_EQUIHASH_N, CORE_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == CORE_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == CORE_EQUIHASH_K, "proof of work 'k' value is incorrect" );
