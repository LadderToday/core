...
#include <core/chain/compound.hpp>
...
#include <core/chain/db_with.hpp>
...
#include <core/chain/steem_evaluator.hpp>
#include <core/chain/steem_objects.hpp>
#include <core/chain/transaction_object.hpp>
#include <core/chain/shared_db_merkle.hpp>
#include <core/chain/operation_notification.hpp>
#include <core/chain/witness_schedule.hpp>

#include <core/chain/util/asset.hpp>
#include <core/chain/util/reward.hpp>
#include <core/chain/util/uint256.hpp>
#include <core/chain/util/reward.hpp>
...
FC_REFLECT( core::chain::object_schema_repr, (space_type)(type) )
FC_REFLECT( core::chain::operation_schema_repr, (id)(type) )
FC_REFLECT( core::chain::db_schema, (types)(object_types)(operation_type)(custom_operation_types) )
...
//      open( data_dir, shared_mem_dir, 0, shared_file_size, chainbase::database::read_write );
      open( data_dir, shared_mem_dir, CORE_INIT_SUPPLY, CORE_INIT_SUPPLY_SBD, shared_file_size, chainbase::database::read_write );
      _fork_db.reset();    // override effect of _fork_db.start_block() call in open()

      auto start = fc::time_point::now();
      CORE_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{ try {
   if( !has_hardfork( CORE_HARDFORK_0_6__127 ) )
...
{
   if( has_hardfork( CORE_HARDFORK_0_17__769 ) || comment.parent_author == CORE_ROOT_POST_PARENT )
...
{
   return get< reward_fund_object, by_name >( CORE_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(CORE_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
   ...
   )
{
   ...
   auto maximum_block_size = get_dynamic_global_properties().maximum_block_size; //CORE_MAX_BLOCK_SIZE;
   ...
   if( has_hardfork( CORE_HARDFORK_0_5__54 ) )
   {
      const auto& witness = get_witness( witness_owner );

      if( witness.running_version != CORE_BLOCKCHAIN_VERSION )
         pending_block.extensions.insert( block_header_extensions( CORE_BLOCKCHAIN_VERSION ) );

      const auto& hfp = get_hardfork_property_object();

      if( hfp.current_hardfork_version < CORE_BLOCKCHAIN_HARDFORK_VERSION // Binary is newer hardfork than has been applied
      ...
      else if( hfp.current_hardfork_version == CORE_BLOCKCHAIN_HARDFORK_VERSION // Binary does not know of a new hardfork
         && witness.hardfork_version_vote > CORE_BLOCKCHAIN_HARDFORK_VERSION ) // Voting for hardfork in the future, that we do not know of...
      {
   ...
   {
      FC_ASSERT( fc::raw::pack_size(pending_block) <= CORE_MAX_BLOCK_SIZE );
   }
...
void database::pop_block()
{
   try
   {
      ...
      CORE_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
void database::notify_pre_apply_operation( operation_notification& note )
{
   ...
   CORE_TRY_NOTIFY( pre_apply_operation, note )
}

void database::notify_post_apply_operation( const operation_notification& note )
{
   CORE_TRY_NOTIFY( post_apply_operation, note )
}
...
{
   CORE_TRY_NOTIFY( applied_block, block )
}

void database::notify_on_pending_transaction( const signed_transaction& tx )
{
   CORE_TRY_NOTIFY( on_pending_transaction, tx )
}

void database::notify_on_pre_apply_transaction( const signed_transaction& tx )
{
   CORE_TRY_NOTIFY( on_pre_apply_transaction, tx )
}

void database::notify_on_applied_transaction( const signed_transaction& tx )
{
   CORE_TRY_NOTIFY( on_applied_transaction, tx )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = CORE_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / CORE_BLOCK_INTERVAL + 1;
}
...
std::pair< asset, asset > database::create_sbd( const account_object& to_account, asset steem, bool to_reward_balance )
{
   ...
   try
   {
      ...
      {
         auto to_sbd = ( gpo.sbd_print_rate * steem.amount ) / CORE_100_PERCENT;
...
uint32_t database::get_pow_summary_target()const
{
   ...
   if( has_hardfork( CORE_HARDFORK_0_16__551 ) )
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   ...
                                   int depth )
{
   if( a.proxy != CORE_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= CORE_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = CORE_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = CORE_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
void database::adjust_witness_vote( const witness_object& witness, share_type delta )
{
   ...
   modify( witness, [&]( witness_object& w )
   {
      ...
      if( has_hardfork( CORE_HARDFORK_0_2 ) )
      ...
      if( has_hardfork( CORE_HARDFORK_0_4 ) )
      {
...
void database::clear_witness_votes( const account_object& a )
{
   ...
   if( has_hardfork( CORE_HARDFORK_0_6__104 ) )
...
{
   if( !has_hardfork( CORE_HARDFORK_0_14__327 ) ) return;

   const auto& null_account = get_account( CORE_NULL_ACCOUNT );
...
{
   if( head_block_num() >= CORE_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
           ...
           ++itr )
      {
         if( itr->auto_vest )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / CORE_100_PERCENT ).to_uint64();
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( CORE_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         while( itr != cvidx.end() && itr->comment == c.id )
         {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               auto reward = create_vesting( voter, asset( claim, STEEM_SYMBOL ), has_hardfork( CORE_HARDFORK_0_17__659 ) );
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment )
{
  try
  {
     ...
     if( comment.net_rshares > 0 )
     {
        ...
        if( has_hardfork( CORE_HARDFORK_0_17__774 ) )
        {
        ...
        const share_type reward = util::get_rshare_reward( ctx, has_hardfork( CORE_HARDFORK_0_20) );
        uint128_t reward_tokens = uint128_t( reward.value );

        if( reward_tokens > 0 )
        {
           share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / CORE_100_PERCENT ).to_uint64();
           ...
           {
              auto benefactor_tokens = ( author_tokens * b.weight ) / CORE_100_PERCENT;
              auto vest_created = create_vesting( get_account( b.account ), benefactor_tokens, has_hardfork( CORE_HARDFORK_0_17__659 ) );
           ...
           auto sbd_steem     = ( author_tokens * comment.percent_steem_dollars ) / ( 2 * CORE_100_PERCENT ) ;

           if( has_hardfork( CORE_HARDFORK_0_20) )
           {
              sbd_steem = 0;
           }
           ...
           auto vest_created = create_vesting( author, vesting_steem, has_hardfork( CORE_HARDFORK_0_17__659 ) );
           auto sbd_payout = create_sbd( author, sbd_steem, has_hardfork( CORE_HARDFORK_0_17__659 ) );
     ...
     modify( comment, [&]( comment_object& c )
     {
         ...
         */
         ...
         if( has_hardfork( CORE_HARDFORK_0_17__769 ) )
         {
            c.cashout_time = fc::time_point_sec::maximum();
         }
         else if( c.parent_author == CORE_ROOT_POST_PARENT )
         {
            if( has_hardfork( CORE_HARDFORK_0_12__177 ) && c.last_payout == fc::time_point_sec::min() )
               c.cashout_time = head_block_time() + CORE_SECOND_CASHOUT_WINDOW;
            else
               c.cashout_time = fc::time_point_sec::maximum();
         }
     ...
     while( vote_itr != vote_idx.end() && vote_itr->comment == comment.id )
     {
        ...
        ++vote_itr;
        if( !has_hardfork( CORE_HARDFORK_0_12__177 ) || calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum() )
        {
...
void database::process_comment_cashout()
{
   ...
   if( !has_hardfork( CORE_FIRST_CASHOUT_TIME ) )
      return;
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      modify( *itr, [&]( reward_fund_object& rfo )
      {
         ...
         if( has_hardfork( CORE_HARDFORK_0_19__1051 ) )
            decay_rate = CORE_RECENT_RSHARES_DECAY_RATE_HF19;
         else
            decay_rate = CORE_RECENT_RSHARES_DECAY_RATE_HF17;
   ...
   if( has_hardfork( CORE_HARDFORK_0_17__771 ) )
   {
...
void database::process_funds()
{
  ...
  if( has_hardfork( CORE_HARDFORK_0_16__551) )
  {
      ...
      */
     int64_t start_inflation_rate = int64_t( CORE_INFLATION_RATE_START_PERCENT );
     int64_t inflation_rate_adjustment = int64_t( head_block_num() / CORE_INFLATION_NARROWING_PERIOD );
     int64_t inflation_rate_floor = int64_t( CORE_INFLATION_RATE_STOP_PERCENT );
     ...
     auto new_steem = ( props.virtual_supply.amount * current_inflation_rate ) / ( int64_t( CORE_100_PERCENT ) * int64_t( CORE_BLOCKS_PER_YEAR ) );
     auto content_reward = ( new_steem * CORE_CONTENT_REWARD_PERCENT ) / CORE_100_PERCENT;
     if( has_hardfork( CORE_HARDFORK_0_17__774 ) )
        content_reward = pay_reward_funds( content_reward ); /// 75% to content creator
     auto vesting_reward = ( new_steem * CORE_VESTING_FUND_PERCENT ) / CORE_100_PERCENT; /// 15% to vesting fund
     auto witness_reward = new_steem - content_reward - vesting_reward; /// Remaining 10% to witness pay

     const auto& cwit = get_witness( props.current_witness );
     witness_reward *= CORE_MAX_WITNESSES;
  ...
  else
  {
     ...
     if( props.head_block_number < CORE_START_VESTING_BLOCK )
...
{
   if( has_hardfork( CORE_HARDFORK_0_12__178 ) )
      return asset( 0, STEEM_SYMBOL );

   const auto& props = get_dynamic_global_properties();
   static_assert( CORE_LIQUIDITY_REWARD_PERIOD_SEC == 60*60, "this code assumes a 1 hour time interval" );
   asset percent( protocol::calc_percent_reward_per_hour< CORE_LIQUIDITY_APR_PERCENT >( props.virtual_supply.amount ), STEEM_SYMBOL );
   return std::max( percent, CORE_MIN_LIQUIDITY_REWARD );
}

asset database::get_content_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( CORE_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< CORE_CONTENT_APR_PERCENT >( props.virtual_supply.amount ), STEEM_SYMBOL );
   return std::max( percent, CORE_MIN_CONTENT_REWARD );
}
...
asset database::get_curation_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( CORE_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< CORE_CURATE_APR_PERCENT >( props.virtual_supply.amount ), STEEM_SYMBOL);
   return std::max( percent, CORE_MIN_CURATE_REWARD );
}
...
asset database::get_producer_reward()
{
   const auto& props = get_dynamic_global_properties();
   static_assert( CORE_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< CORE_PRODUCER_APR_PERCENT >( props.virtual_supply.amount ), STEEM_SYMBOL);
   auto pay = std::max( percent, CORE_MIN_PRODUCER_REWARD );
   const auto& witness_account = get_account( props.current_witness );

   /// pay witness in vesting shares
   if( props.head_block_number >= CORE_START_MINER_VOTING_BLOCK || (witness_account.vesting_shares.amount.value == 0) ) {
...
#ifndef IS_TEST_NET
   /// 0 block rewards until at least CORE_MAX_WITNESSES have produced a POW
   if( props.num_pow_witnesses < CORE_MAX_WITNESSES && props.head_block_number < CORE_START_VESTING_BLOCK )
      return asset( 0, STEEM_SYMBOL );
#endif

   static_assert( CORE_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
// tuanpa - temporary for private testnet only
//   static_assert( CORE_MAX_WITNESSES == 21, "this code assumes 21 per round" );
   asset percent( calc_percent_reward_per_round< CORE_POW_APR_PERCENT >( props.virtual_supply.amount ), STEEM_SYMBOL);
   return std::max( percent, CORE_MIN_POW_REWARD );
}
...
void database::pay_liquidity_reward()
{
   ...
   if( (head_block_num() % CORE_LIQUIDITY_REWARD_BLOCKS) == 0 )
   {
...
uint16_t database::get_curation_rewards_percent( const comment_object& c ) const
{
   ...
   else if( has_hardfork( CORE_HARDFORK_0_8__116 ) )
      return CORE_1_PERCENT * 25;
   else
      return CORE_1_PERCENT * 50;
}
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / CORE_100_PERCENT;
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + CORE_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      std::array<share_type, CORE_MAX_PROXY_RECURSION_DEPTH+1> delta;
      delta[0] = -account.vesting_shares.amount;
      for( int i = 0; i < CORE_MAX_PROXY_RECURSION_DEPTH; ++i )
      ...
      {
         a.can_vote = false;
         a.proxy = CORE_PROXY_TO_SELF_ACCOUNT;
      });
...
void database::init_genesis( uint64_t init_supply, uint64_t init_supply_sbd )
{
   try
   {
      ...
      public_key_type      init_public_key(CORE_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = CORE_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = CORE_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = CORE_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = CORE_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
      });
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = CORE_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = CORE_TEMP_ACCOUNT;
         auth.owner.weight_threshold = 0;
         auth.active.weight_threshold = 0;
      });

      for( int i = 0; i < CORE_NUM_INIT_MINERS; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = CORE_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            ...
            // Add CORE_INIT_SBD_SUPPLY config option
         ...
         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = CORE_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = CORE_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = CORE_INIT_MINER_NAME;
         p.time = CORE_GENESIS_TIME;
...
//         p.virtual_supply = p.current_supply + asset( init_supply_sbd, STEEM_SYMBOL );
         ...
         p.maximum_block_size = CORE_MAX_BLOCK_SIZE;
      } );
      ...
      {
         hpo.processed_hardforks.push_back( CORE_GENESIS_TIME );
      } );

      // Create witness scheduler
      create< witness_schedule_object >( [&]( witness_schedule_object& wso )
      {
         wso.current_shuffled_witnesses[0] = CORE_INIT_MINER_NAME;
      } );
   }
   FC_CAPTURE_AND_RETHROW()
}
...
void database::notify_changed_objects()
{
   try
   {
      ...
      get_changed_ids( ids );
      CORE_TRY_NOTIFY( changed_objects, ids )*/
      /*
      if( _undo_db.enabled() )
      {
         ...
         CORE_TRY_NOTIFY( changed_objects, changed_ids )
      }
      */
   }
   FC_CAPTURE_AND_RETHROW()
...
{ try {
   ...
   if( has_hardfork( CORE_HARDFORK_0_12 ) )
   {
   ...
   if( block_size < CORE_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",CORE_MIN_BLOCK_SIZE)
      );
   }
   ...
   if( has_hardfork( CORE_HARDFORK_0_5__54 ) ) // Cannot remove after hardfork
   {
...
void database::update_median_feed() {
  try {
     ...
     if ((head_block_num() % CORE_FEED_INTERVAL_BLOCKS) != 0)
        return;
     ...
     for (int i = 0; i < wso.num_scheduled_witnesses; i++) {
        ...
        if (has_hardfork(CORE_HARDFORK_0_19__822)) {
           if (now < wit.last_sbd_exchange_update + CORE_MAX_FEED_AGE_SECONDS
               && !wit.sbd_exchange_rate.is_null()) {
              feeds.push_back(wit.sbd_exchange_rate);
           }
        } else if (wit.last_sbd_exchange_update < now + CORE_MAX_FEED_AGE_SECONDS &&
                   !wit.sbd_exchange_rate.is_null()) {
           feeds.push_back(wit.sbd_exchange_rate);
        }
     }

     if (feeds.size() >= CORE_MIN_FEEDS) {
        std::sort(feeds.begin(), feeds.end());
        auto median_feed = feeds[feeds.size() / 2];

        modify(get_feed_history(), [&](feed_history_object &fho) {
            fho.price_history.push_back(median_feed);
            size_t steemit_feed_history_window = CORE_FEED_HISTORY_WINDOW_PRE_HF_16;
            if (has_hardfork(CORE_HARDFORK_0_16__551))
               steemit_feed_history_window = CORE_FEED_HISTORY_WINDOW;
          ...
          return;
#endif
               if (has_hardfork(CORE_HARDFORK_0_14__230)) {
                  const auto &gpo = get_dynamic_global_properties();
...
{ try {
   ...
   const chain_id_type& chain_id = CORE_CHAIN_ID;
   auto trx_id = trx.id();
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, CORE_MAX_SIG_CHECK_DEPTH );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         CORE_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      CORE_ASSERT( trx.expiration <= now + fc::seconds(CORE_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",CORE_MAX_TIME_UNTIL_EXPIRATION));
      if( has_hardfork( CORE_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when now == trx.expiration
         CORE_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      CORE_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
...
{ try {
   ...
   if( head_block_time() != fc::time_point_sec() )
   {
      ...
      for( uint32_t i = 0; i < missed_blocks; ++i )
      {
         ...
         if(  witness_missed.owner != b.witness )
         {
            ...
            {
               w.total_missed++;
               if( has_hardfork( CORE_HARDFORK_0_14__278 ) )
               {
                  if( head_block_num() - w.last_confirmed_block_num  > CORE_BLOCKS_PER_DAY )
                  {
   ...
   {
      CORE_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < CORE_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",CORE_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
     modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
     {
         if( has_hardfork( CORE_HARDFORK_0_20) )
         {
            ...
            if( !median_price.is_null() && has_hardfork( CORE_HARDFORK_0_14__230 ) )
            {
               auto percent_sbd = uint16_t( ( ( fc::uint128_t( ( dgp.current_sbd_supply * get_feed_history().current_median_history ).amount.value ) * CORE_100_PERCENT )
                                              / dgp.virtual_supply.amount.value ).to_uint64() );

               if( percent_sbd <= CORE_SBD_START_PERCENT )
                  dgp.sbd_print_rate = CORE_100_PERCENT;
               else if( percent_sbd >= CORE_SBD_STOP_PERCENT )
                  dgp.sbd_print_rate = 0;
               else
                  dgp.sbd_print_rate = ( ( CORE_SBD_STOP_PERCENT - percent_sbd ) * CORE_100_PERCENT ) / ( CORE_SBD_STOP_PERCENT - CORE_SBD_START_PERCENT );
            }
         }
     });
  } FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < CORE_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > CORE_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - CORE_MAX_WITNESSES;
      } );
   }
   else
   {
      ...
      static_assert( CORE_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((CORE_100_PERCENT - CORE_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / CORE_100_PERCENT);
...
int database::match( const limit_order_object& new_order, const limit_order_object& old_order, const price& match_price )
{
   ...
   if( !has_hardfork( CORE_HARDFORK_0_12__178 ) &&
       ( (age >= CORE_MIN_LIQUIDITY_REWARD_PERIOD_SEC && !has_hardfork( CORE_HARDFORK_0_10__149)) ||
       (age >= CORE_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10 && has_hardfork( CORE_HARDFORK_0_10__149) ) ) )
   {
...
void database::adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_sdb )
{
   ...
   if( itr != ridx.end() )
   {
      ...
      {
         if( head_block_time() - r.last_update >= CORE_LIQUIDITY_TIMEOUT_SEC )
         {
            r.sbd_volume = 0;
            r.steem_volume = 0;
            r.weight = 0;
         ...
         r.update_weight( has_hardfork( CORE_HARDFORK_0_10__141 ) );
   ...
   else
   {
      create<liquidity_reward_balance_object>( [&](liquidity_reward_balance_object& r )
      {
         ...
         r.update_weight( has_hardfork( CORE_HARDFORK_0_9__141 ) );
         r.last_update = head_block_time();
      } );
   }
}
         ...
         case SBD_SYMBOL:
            if( a.sbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.sbd_seconds > 0 &&
                   (acnt.sbd_seconds_last_update - acnt.sbd_last_interest_payment).to_seconds() > CORE_SBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.sbd_seconds / CORE_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().sbd_interest_rate;
                  interest /= CORE_100_PERCENT;
         ...
         case SBD_SYMBOL:
            if( a.savings_sbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.savings_sbd_seconds > 0 &&
                   (acnt.savings_sbd_seconds_last_update - acnt.savings_sbd_last_interest_payment).to_seconds() > CORE_SBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.savings_sbd_seconds / CORE_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().sbd_interest_rate;
                  interest /= CORE_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), SBD_SYMBOL);
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < CORE_BLOCKS_PER_DAY*7 )
...
{
   _hardfork_times[ 0 ] = fc::time_point_sec( CORE_GENESIS_TIME );
   _hardfork_versions[ 0 ] = hardfork_version( 0, 0 );
   FC_ASSERT( CORE_HARDFORK_0_1 == 1, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_1 ] = fc::time_point_sec( CORE_HARDFORK_0_1_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_1 ] = CORE_HARDFORK_0_1_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_2 == 2, "Invlaid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_2 ] = fc::time_point_sec( CORE_HARDFORK_0_2_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_2 ] = CORE_HARDFORK_0_2_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_3 == 3, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_3 ] = fc::time_point_sec( CORE_HARDFORK_0_3_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_3 ] = CORE_HARDFORK_0_3_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_4 == 4, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_4 ] = fc::time_point_sec( CORE_HARDFORK_0_4_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_4 ] = CORE_HARDFORK_0_4_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_5 == 5, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_5 ] = fc::time_point_sec( CORE_HARDFORK_0_5_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_5 ] = CORE_HARDFORK_0_5_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_6 == 6, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_6 ] = fc::time_point_sec( CORE_HARDFORK_0_6_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_6 ] = CORE_HARDFORK_0_6_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_7 == 7, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_7 ] = fc::time_point_sec( CORE_HARDFORK_0_7_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_7 ] = CORE_HARDFORK_0_7_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_8 == 8, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_8 ] = fc::time_point_sec( CORE_HARDFORK_0_8_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_8 ] = CORE_HARDFORK_0_8_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_9 == 9, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_9 ] = fc::time_point_sec( CORE_HARDFORK_0_9_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_9 ] = CORE_HARDFORK_0_9_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_10 == 10, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_10 ] = fc::time_point_sec( CORE_HARDFORK_0_10_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_10 ] = CORE_HARDFORK_0_10_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_11 == 11, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_11 ] = fc::time_point_sec( CORE_HARDFORK_0_11_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_11 ] = CORE_HARDFORK_0_11_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_12 == 12, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_12 ] = fc::time_point_sec( CORE_HARDFORK_0_12_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_12 ] = CORE_HARDFORK_0_12_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_13 == 13, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_13 ] = fc::time_point_sec( CORE_HARDFORK_0_13_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_13 ] = CORE_HARDFORK_0_13_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_14 == 14, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_14 ] = fc::time_point_sec( CORE_HARDFORK_0_14_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_14 ] = CORE_HARDFORK_0_14_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_15 == 15, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_15 ] = fc::time_point_sec( CORE_HARDFORK_0_15_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_15 ] = CORE_HARDFORK_0_15_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_16 == 16, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_16 ] = fc::time_point_sec( CORE_HARDFORK_0_16_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_16 ] = CORE_HARDFORK_0_16_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_17 == 17, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_17 ] = fc::time_point_sec( CORE_HARDFORK_0_17_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_17 ] = CORE_HARDFORK_0_17_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_18 == 18, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_18 ] = fc::time_point_sec( CORE_HARDFORK_0_18_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_18 ] = CORE_HARDFORK_0_18_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_19 == 19, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_19 ] = fc::time_point_sec( CORE_HARDFORK_0_19_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_19 ] = CORE_HARDFORK_0_19_VERSION;
   FC_ASSERT( CORE_HARDFORK_0_20 == 20, "Invalid hardfork configuration" );
   _hardfork_times[ CORE_HARDFORK_0_20 ] = fc::time_point_sec( CORE_HARDFORK_0_20_TIME );
   _hardfork_versions[ CORE_HARDFORK_0_20 ] = CORE_HARDFORK_0_20_VERSION;

   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= CORE_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("CORE_NUM_HARDFORKS",CORE_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions[ hardforks.last_hardfork ] <= CORE_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( CORE_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions[ CORE_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      if( has_hardfork( CORE_HARDFORK_0_5__54 ) )
      {
         ...
         {
            if( hardforks.last_hardfork < CORE_NUM_HARDFORKS ) {
      ...
      {
         while( hardforks.last_hardfork < CORE_NUM_HARDFORKS
               && _hardfork_times[ hardforks.last_hardfork + 1 ] <= head_block_time()
               && hardforks.last_hardfork < CORE_HARDFORK_0_5__54 )
         {
            apply_hardfork( hardforks.last_hardfork + 1 );
         }
      }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= CORE_NUM_HARDFORKS; i++ )
   {
      if( i <= CORE_HARDFORK_0_5__54 )
   ...
   switch( hardfork )
   {
      case CORE_HARDFORK_0_1:
...
#ifdef IS_TEST_NET
         {
            ...
            test_op.required_auths.insert( CORE_INIT_MINER_NAME );
...
#endif
         break;
      case CORE_HARDFORK_0_2:
         retally_witness_votes();
         break;
      case CORE_HARDFORK_0_3:
         retally_witness_votes();
         break;
      case CORE_HARDFORK_0_4:
         reset_virtual_schedule_time(*this);
         break;
      case CORE_HARDFORK_0_5:
         break;
      case CORE_HARDFORK_0_6:
         retally_witness_vote_counts();
         retally_comment_children();
         break;
      case CORE_HARDFORK_0_7:
         break;
      case CORE_HARDFORK_0_8:
         retally_witness_vote_counts(true);
         break;
      case CORE_HARDFORK_0_9:
         break;
      case CORE_HARDFORK_0_10:
         retally_liquidity_weight();
         break;
      case CORE_HARDFORK_0_11:
         break;
      case CORE_HARDFORK_0_12:
         {
            ...
            for( auto itr = comment_idx.begin(); itr != comment_idx.end(); ++itr )
            {
               ...
               if( itr->parent_author == CORE_ROOT_POST_PARENT )
               {
                  ...
                  if( itr->last_payout == fc::time_point_sec::min() && itr->cashout_time == fc::time_point_sec::maximum() )
                  {
                     ...
                     {
                        c.cashout_time = head_block_time() + CORE_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                     });
                  }
                  ...
                  else if( itr->last_payout > fc::time_point_sec() )
                  {
                     modify( *itr, [&]( comment_object& c )
                     {
                        c.cashout_time = c.last_payout + CORE_SECOND_CASHOUT_WINDOW;
                     });
                  }
               }
            }

            modify( get< account_authority_object, by_account >( CORE_MINER_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( CORE_NULL_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( CORE_TEMP_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });
         ...
         break;
      case CORE_HARDFORK_0_13:
         break;
      case CORE_HARDFORK_0_14:
         break;
      case CORE_HARDFORK_0_15:
         break;
      case CORE_HARDFORK_0_16:
         {
            modify( get_feed_history(), [&]( feed_history_object& fho )
            {
               while( fho.price_history.size() > CORE_FEED_HISTORY_WINDOW )
                  fho.price_history.pop_front();
            });
         }
         break;
      case CORE_HARDFORK_0_17:
         {
            static_assert(
               CORE_MAX_VOTED_WITNESSES_HF0 + CORE_MAX_MINER_WITNESSES_HF0 + CORE_MAX_RUNNER_WITNESSES_HF0 == CORE_MAX_WITNESSES,
               "HF0 witness counts must add up to CORE_MAX_WITNESSES" );
            static_assert(
               CORE_MAX_VOTED_WITNESSES_HF17 + CORE_MAX_MINER_WITNESSES_HF17 + CORE_MAX_RUNNER_WITNESSES_HF17 == CORE_MAX_WITNESSES,
               "HF17 witness counts must add up to CORE_MAX_WITNESSES" );

            modify( get_witness_schedule_object(), [&]( witness_schedule_object& wso )
            {
               wso.max_voted_witnesses = CORE_MAX_VOTED_WITNESSES_HF17;
               wso.max_miner_witnesses = CORE_MAX_MINER_WITNESSES_HF17;
               wso.max_runner_witnesses = CORE_MAX_RUNNER_WITNESSES_HF17;
            });
            ...
            {
               rfo.name = CORE_POST_REWARD_FUND_NAME;
               rfo.last_update = head_block_time();
               rfo.content_constant = CORE_CONTENT_CONSTANT_HF0;
               rfo.percent_curation_rewards = CORE_1_PERCENT * 25;
               rfo.percent_content_rewards = CORE_100_PERCENT;
               rfo.reward_balance = gpo.total_reward_fund_steem;
#ifndef IS_TEST_NET
               rfo.recent_claims = CORE_HF_17_RECENT_CLAIMS;
#endif
            ...
            */
            ...
            root_posts.reserve( CORE_HF_17_NUM_POSTS );
            vector< const comment_object* > replies;
            replies.reserve( CORE_HF_17_NUM_REPLIES );
            ...
            for( auto itr : root_posts )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( c.created + CORE_CASHOUT_WINDOW_SECONDS, c.cashout_time );
               });
            }
            ...
            for( auto itr : replies )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( calculate_discussion_payout_time( c ), c.created + CORE_CASHOUT_WINDOW_SECONDS );
               });
            }
         }
         break;
      case CORE_HARDFORK_0_18:
         break;
      case CORE_HARDFORK_0_19:
         {
            ...
            modify( get< reward_fund_object, by_name >( CORE_POST_REWARD_FUND_NAME ), [&]( reward_fund_object &rfo )
            {
#ifndef IS_TEST_NET
               rfo.recent_claims = CORE_HF_19_RECENT_CLAIMS;
#endif
...
void database::validate_invariants()const
{
  try
  {
     ...
     for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
     {
        ...
        total_vsf_votes += ( itr->proxy == CORE_PROXY_TO_SELF_ACCOUNT ?
                             itr->witness_vote_weight() :
                             ( CORE_MAX_PROXY_RECURSION_DEPTH > 0 ?
                               itr->proxied_vsf_votes[CORE_MAX_PROXY_RECURSION_DEPTH - 1] :
   ...
   try
   {
      ...
      for( const auto& account : get_index<account_index>().indices() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.vesting_withdraw_rate  = asset( a.to_withdraw / CORE_VESTING_WITHDRAW_INTERVALS_PRE_HF_16, VESTS_SYMBOL );
            if( a.vesting_withdraw_rate.amount == 0 )
               a.vesting_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < CORE_MAX_PROXY_RECURSION_DEPTH; ++i )
...
// Low memory nodes only need immediate child count, full nodes track total children
         ...
         while( parent )
         {
            ...
            if( parent->parent_author != CORE_ROOT_POST_PARENT )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != CORE_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != CORE_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.id, witness_id_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.id )
