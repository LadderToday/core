...
(see `libraries/plugins/debug_node/include/core/plugins/debug_node/debug_node_api.hpp`
...
Okay, let's run `cored`.  It should start immediately with no blocks.  We can ask it to read blocks from the directory we saved earlier:
...
If we want to take control of an account we can do so by editing its key with `debug_update_object` command like this:
    ...
    curl --data '{"jsonrpc": "2.0", "method": "call", "params": [0,"lookup_account_names",[["core"]]], "id": 8}' http://127.0.0.1:8090/rpc    # find out ID of account we want is 2.2.28
...
Now that we've reset its key, we can take control of it in the wallet:
    ...
    list_my_accounts
    transfer core dantheman "1.234 CORE" "make -j100 money" true
    list_my_accounts
    get_account_history core -1 1000
