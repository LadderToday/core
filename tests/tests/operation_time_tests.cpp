...
#include <core/plugins/debug_node/debug_node_plugin.hpp>
...
BOOST_AUTO_TEST_CASE( comment_payout_equalize )
{
   try
   {
      ...
      for( const auto& author : authors )
      {
         ...
         com.permlink = "mypost";
         com.parent_author = CORE_ROOT_POST_PARENT;
      ...
      generate_blocks(10);
      //const auto& rf = db.get< reward_fund_object, by_name >( CORE_POST_REWARD_FUND_NAME );
      //idump( (rf) );
...
BOOST_AUTO_TEST_CASE( reward_funds )
{
   try
   {
      ...
      {
         const auto& post_rf = db.get< reward_fund_object, by_name >( CORE_POST_REWARD_FUND_NAME );
         const auto& comment_rf = db.get< reward_fund_object, by_name >( CORE_COMMENT_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( recent_claims_decay )
{
   try
   {
      ...
      auto alice_vshares = util::evaluate_reward_curve( db.get_comment( "alice", string( "test" ) ).net_rshares.value,
         db.get< reward_fund_object, by_name >( CORE_POST_REWARD_FUND_NAME ).author_reward_curve,
         db.get< reward_fund_object, by_name >( CORE_POST_REWARD_FUND_NAME ).content_constant );
      ...
      {
         alice_vshares -= ( alice_vshares * CORE_BLOCK_INTERVAL ) / CORE_RECENT_RSHARES_DECAY_RATE_HF19.to_seconds();
         const auto& post_rf = db.get< reward_fund_object, by_name >( CORE_POST_REWARD_FUND_NAME );
...
/*BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      //generate_blocks( db.get_comment( "bob", string( "test" ) ).cashout_time - CORE_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      generate_blocks( fc::time_point_sec( db.head_block_time().sec_since_epoch() + CORE_CASHOUT_WINDOW_SECONDS / 2 ), true );
      ...
      generate_blocks( db.get_comment( "bob", string( "test" ) ).cashout_time - ( CORE_BLOCK_INTERVAL / 2 ), true );
      ...
      generate_blocks( db.get_comment( "alice", string( "test" ) ).cashout_time - ( CORE_BLOCK_INTERVAL / 2 ), true );
...
BOOST_AUTO_TEST_CASE( nested_comments )
{
   try
   {
      ...
      tx.operations.push_back( vote_op );
      vote_op.author = "dave";
      vote_op.weight = CORE_1_PERCENT * 20;
      tx.operations.push_back( vote_op );
      tx.sign( bob_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
      ...
      generate_blocks( db.get_comment( "alice", string( "test" ) ).cashout_time - fc::seconds( CORE_BLOCK_INTERVAL ), true );
...
BOOST_AUTO_TEST_CASE( vesting_withdrawals )
{
   try
   {
      ...
      auto next_withdrawal = db.head_block_time() + CORE_VESTING_WITHDRAW_INTERVAL_SECONDS;
      ...
      generate_blocks( next_withdrawal - ( CORE_BLOCK_INTERVAL / 2 ), true);
      ...
      for( int i = 1; i < CORE_VESTING_WITHDRAW_INTERVALS - 1; i++ )
      {
         generate_blocks( db.head_block_time() + CORE_VESTING_WITHDRAW_INTERVAL_SECONDS );
         ...
         if ( i == CORE_VESTING_WITHDRAW_INTERVALS - 1 )
            BOOST_REQUIRE( alice.next_vesting_withdrawal == fc::time_point_sec::maximum() );
         else
            BOOST_REQUIRE( alice.next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + CORE_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
      ...
      if (  to_withdraw.amount.value % withdraw_rate.amount.value != 0 )
      {
         ...
         generate_blocks( db.head_block_time() + CORE_VESTING_WITHDRAW_INTERVAL_SECONDS, true );
         fill_op = get_last_operations( 1 )[0].get< fill_vesting_withdraw_operation >();
         gpo = db.get_dynamic_global_properties();

         BOOST_REQUIRE( db.get_account( "alice" ).next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + CORE_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
         BOOST_REQUIRE( fill_op.from_account == "alice" );
         BOOST_REQUIRE( fill_op.to_account == "alice" );
         BOOST_REQUIRE( fill_op.withdrawn.amount.value == withdraw_rate.amount.value );
         BOOST_REQUIRE( std::abs( ( fill_op.deposited - fill_op.withdrawn * gpo.get_vesting_share_price() ).amount.value ) <= 1 );
...
BOOST_AUTO_TEST_CASE( vesting_withdraw_route )
{
   try
   {
      ...
      op.to_account = "bob";
      op.percent = CORE_1_PERCENT * 50;
      op.auto_vest = true;
      tx.operations.push_back( op );

      BOOST_TEST_MESSAGE( "Setting up sam destination" );
      op.to_account = "sam";
      op.percent = CORE_1_PERCENT * 30;
      op.auto_vest = false;
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
...

      {
         ...
         BOOST_REQUIRE( alice.balance == old_alice_balance + asset( ( vesting_withdraw_rate.amount * CORE_1_PERCENT * 20 ) / CORE_100_PERCENT, VESTS_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
         BOOST_REQUIRE( bob.vesting_shares == old_bob_vesting + asset( ( vesting_withdraw_rate.amount * CORE_1_PERCENT * 50 ) / CORE_100_PERCENT, VESTS_SYMBOL ) );
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * CORE_1_PERCENT * 30 ) / CORE_100_PERCENT, VESTS_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
      ...
      op.to_account = "sam";
      op.percent = CORE_1_PERCENT * 50 + 1;
      tx.operations.push_back( op );
      tx.set_expiration( db.head_block_time() + CORE_MAX_TIME_UNTIL_EXPIRATION );
      tx.sign( alice_private_key, db.get_chain_id() );
      ...
      generate_blocks( db.get_account( "alice" ).next_vesting_withdrawal, true );
      {
         ...
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * CORE_1_PERCENT * 50 ) / CORE_100_PERCENT, VESTS_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
      }
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_mean )
{
   try
   {
      ...
      generate_blocks( 30 / CORE_BLOCK_INTERVAL );
      ...
      for( int i = 0; i < 7; i++ )
      {
         transfer( CORE_INIT_MINER_NAME, accounts[i], 10000 );
      ...
      for( int i = 0; i < 7; i++ )
      {
         txs[i].set_expiration( db.head_block_time() + CORE_MAX_TIME_UNTIL_EXPIRATION );
      ...
      generate_blocks( CORE_BLOCKS_PER_HOUR ); // Jump forward 1 hour
      ...
      for ( int i = 0; i < 23; i++ )
      {
         ...
         for( int j = 0; j < 7; j++ )
         {
            ...
            txs[j].set_expiration( db.head_block_time() + CORE_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( convert_delay )
{
   try
   {
      ...
      generate_blocks( db.head_block_time() + CORE_CONVERSION_DELAY - fc::seconds( CORE_BLOCK_INTERVAL / 2 ), true );
...
BOOST_AUTO_TEST_CASE( steem_inflation )
{
   ...
   /*
      ...
      auto new_rewards = std::max( CORE_MIN_CONTENT_REWARD, asset( ( CORE_CONTENT_APR * gpo.virtual_supply.amount ) / ( CORE_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) )
         + std::max( CORE_MIN_CURATE_REWARD, asset( ( CORE_CURATE_APR * gpo.virtual_supply.amount ) / ( CORE_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) );
      auto witness_pay = std::max( CORE_MIN_PRODUCER_REWARD, asset( ( CORE_PRODUCER_APR * gpo.virtual_supply.amount ) / ( CORE_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) );
      ...
      while( db.head_block_num() < CORE_START_VESTING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( CORE_MIN_CONTENT_REWARD, asset( ( CORE_CONTENT_APR * gpo.virtual_supply.amount ) / ( CORE_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) )
            + std::max( CORE_MIN_CURATE_REWARD, asset( ( CORE_CURATE_APR * gpo.virtual_supply.amount ) / ( CORE_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) );
         witness_pay = std::max( CORE_MIN_PRODUCER_REWARD, asset( ( CORE_PRODUCER_APR * gpo.virtual_supply.amount ) / ( CORE_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) );
      ...
      while( db.head_block_num() < CORE_START_MINER_VOTING_BLOCK - 1 )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db.get_scheduled_witness(1);
         old_witness_balance = db.get_account( witness_name ).balance;
      ...
      for( int i = 0; i < CORE_BLOCKS_PER_DAY; i++ )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db.get_scheduled_witness(1);
         old_witness_balance = db.get_account( witness_name ).balance;
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_vesting_fund_steem.amount.value,
         ( vesting_steem.amount.value
            + ( ( ( uint128_t( virtual_supply.amount.value ) / 10 ) / CORE_BLOCKS_PER_YEAR ) * 9 )
            + ( uint128_t( virtual_supply.amount.value ) / 100 / CORE_BLOCKS_PER_YEAR ) ).to_uint64() );
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_steem.amount.value,
         reward_steem.amount.value + virtual_supply.amount.value / 10 / CORE_BLOCKS_PER_YEAR + virtual_supply.amount.value / 10 / CORE_BLOCKS_PER_DAY );
      BOOST_REQUIRE_EQUAL( db.get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / CORE_BLOCKS_PER_YEAR ) * CORE_1_PERCENT ) / CORE_100_PERCENT, STEEM_SYMBOL ) * ( vesting_shares / vesting_steem ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE( sbd_interest )
{
   try
   {
      ...
      generate_blocks( db.head_block_time() + fc::seconds( CORE_SBD_INTEREST_COMPOUND_INTERVAL_SEC ), true );
      ...
      BOOST_REQUIRE( db.get_account( "alice" ).sbd_balance.amount.value == alice_sbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_sbd.amount.value ) * ( db.head_block_time() - start_time ).to_seconds() ) / CORE_SECONDS_PER_YEAR ) * gpo.sbd_interest_rate ) / CORE_100_PERCENT ).to_uint64() );
      ...
      generate_blocks( db.head_block_time() + fc::seconds( CORE_SBD_INTEREST_COMPOUND_INTERVAL_SEC / 2 ), true );
      ...
      generate_blocks( db.head_block_time() + fc::seconds( ( CORE_SBD_INTEREST_COMPOUND_INTERVAL_SEC * 7 ) / 3 ), true );
      ...
      BOOST_REQUIRE( db.get_account( "alice" ).sbd_balance.amount.value == alice_sbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_sbd.amount.value ) * ( db.head_block_time() - start_time ).to_seconds() + alice_coindays ) / CORE_SECONDS_PER_YEAR ) * gpo.sbd_interest_rate ) / CORE_100_PERCENT ).to_uint64() );
      validate_database();
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( liquidity_rewards )
{
   ...
   try
   {
      ...
      generate_blocks( db.head_block_time() + CORE_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10, true );
      ...
      generate_blocks( db.head_block_time() + fc::seconds( CORE_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10.to_seconds() / 2 ), true );
      ...
      generate_blocks( CORE_LIQUIDITY_REWARD_BLOCKS - ( db.head_block_num() % CORE_LIQUIDITY_REWARD_BLOCKS ) - 1 );

      BOOST_REQUIRE( db.head_block_num() % CORE_LIQUIDITY_REWARD_BLOCKS == CORE_LIQUIDITY_REWARD_BLOCKS - 1 );
      ...
      //alice_balance += CORE_MIN_LIQUIDITY_REWARD;
      ...
      CORE_REQUIRE_THROW( ops[0].get< liquidity_reward_operation>(), fc::exception );
      //BOOST_REQUIRE( ops[0].get< liquidity_reward_operation>().payout.amount.value == CORE_MIN_LIQUIDITY_REWARD.amount.value );

      generate_blocks( CORE_LIQUIDITY_REWARD_BLOCKS );

      //bob_balance += CORE_MIN_LIQUIDITY_REWARD;
      ...
      generate_blocks( sam_reward_last_update + CORE_LIQUIDITY_TIMEOUT_SEC - fc::seconds( CORE_BLOCK_INTERVAL / 2 ) - CORE_MIN_LIQUIDITY_REWARD_PERIOD_SEC , true );
      ...
      generate_blocks( db.head_block_time() + ( CORE_BLOCK_INTERVAL / 2 ) + CORE_LIQUIDITY_TIMEOUT_SEC, true );
...
BOOST_AUTO_TEST_CASE( post_rate_limit )
{
   try
   {
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test1" ) ).reward_weight == CORE_100_PERCENT );

      tx.operations.clear();
      tx.signatures.clear();

      generate_blocks( db.head_block_time() + CORE_MIN_ROOT_COMMENT_INTERVAL + fc::seconds( CORE_BLOCK_INTERVAL ), true );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test2" ) ).reward_weight == CORE_100_PERCENT );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test3" ) ).reward_weight == CORE_100_PERCENT );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test4" ) ).reward_weight == CORE_100_PERCENT );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test5" ) ).reward_weight == CORE_100_PERCENT );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_freeze )
{
   try
   {
      ...
      vote.voter = "bob";
      vote.weight = CORE_100_PERCENT * -1;
...
BOOST_AUTO_TEST_CASE( sbd_stability )
{
   try
   {
      ...
      auto db_plugin = app.register_plugin< core::plugin::debug_node::debug_node_plugin >();
      ...
      BOOST_REQUIRE( db.get_dynamic_global_properties().sbd_print_rate == CORE_100_PERCENT );
      ...
      db_plugin->debug_generate_blocks_until( debug_key, fc::time_point_sec( db.get_comment( comment.author, comment.permlink ).cashout_time.sec_since_epoch() - 2 * CORE_BLOCK_INTERVAL ), true, database::skip_witness_signature );

      auto& gpo = db.get_dynamic_global_properties();

      BOOST_TEST_MESSAGE( "Changing sam and gpo to set up market cap conditions" );

      asset sbd_balance = asset( ( gpo.virtual_supply.amount * ( CORE_SBD_STOP_PERCENT + 30 ) ) / CORE_100_PERCENT, STEEM_SYMBOL ) * exchange_rate;
      ...
      auto comment_reward = ( gpo.total_reward_fund_steem.amount + 2000 ) - ( ( gpo.total_reward_fund_steem.amount + 2000 ) * 25 * CORE_1_PERCENT ) / CORE_100_PERCENT ;
      comment_reward /= 2;
      auto sbd_reward = ( comment_reward * gpo.sbd_print_rate ) / CORE_100_PERCENT;
      ...
      while( ( db.get_dynamic_global_properties().current_sbd_supply * exchange_rate ).amount >= ( db.get_dynamic_global_properties().virtual_supply.amount * CORE_SBD_START_PERCENT ) / CORE_100_PERCENT )
      {
         auto& gpo = db.get_dynamic_global_properties();
...
BOOST_AUTO_TEST_CASE( sbd_price_feed_limit )
{
   try
   {
      ...
      auto new_exchange_rate = price( gpo.current_sbd_supply, asset( ( CORE_100_PERCENT ) * gpo.current_supply.amount ) );
...
BOOST_AUTO_TEST_CASE( clear_null_account )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( CORE_NULL_ACCOUNT ), [&]( account_object& a )
         {
      ...
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).sbd_balance == ASSET( "2.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).vesting_shares > ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).savings_balance == ASSET( "4.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).savings_sbd_balance == ASSET( "5.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).reward_sbd_balance == ASSET( "1.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).reward_steem_balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "1.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).reward_vesting_steem == ASSET( "1.000 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).vesting_shares == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).savings_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).savings_sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).reward_sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).reward_steem_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( CORE_NULL_ACCOUNT ).reward_vesting_steem == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( "alice" ).balance == ASSET( "2.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( "alice" ).sbd_balance == ASSET( "3.000 TBD" ) );
   }
   FC_LOG_AND_RETHROW()
