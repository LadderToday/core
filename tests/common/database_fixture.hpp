...
extern uint32_t ( CORE_TESTING_GENESIS_TIMESTAMP );

#define PUSH_TX \
   core::chain::test::_push_transaction

#define PUSH_BLOCK \
   core::chain::test::_push_block
...
/*#define CORE_REQUIRE_THROW( expr, exc_type )          \
{                                                         \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CORE_REQUIRE_THROW begin "        \
         << req_throw_info << std::endl;                  \
   BOOST_REQUIRE_THROW( expr, exc_type );                 \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CORE_REQUIRE_THROW end "          \
         << req_throw_info << std::endl;                  \
}*/
...
#define CORE_CHECK_THROW( expr, exc_type )            \
{                                                         \
   std::string req_throw_info = fc::json::to_string(      \
      fc::mutable_variant_object()                        \
      ("source_file", __FILE__)                           \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CORE_CHECK_THROW begin "          \
         << req_throw_info << std::endl;                  \
   BOOST_CHECK_THROW( expr, exc_type );                   \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CORE_CHECK_THROW end "            \
         << req_throw_info << std::endl;                  \
}
...
{ \
   const auto temp = op.field; \
   op.field = value; \
   CORE_REQUIRE_THROW( op.validate(), exc_type ); \
   op.field = temp; \
}
...
{ \
   ...
   trx.operations.back() = op; \
   op.field = bak; \
   CORE_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
struct database_fixture {
   ...
   std::shared_ptr< core::plugin::debug_node::debug_node_plugin > db_plugin;
